      ...
      return `
            <div class="event-card">
                ...
                <div class="event-actions">
                    <button class="btn primary" onclick="selectEvent(${
                      event.id
                    }, '${event.nombre || event.title}', ${
        event.precio || event.price
      })">
...
async function loadNotifications() {
  ...
  try {
    const response = await fetch(
      `${API_ENDPOINTS.notificaciones}/usuario/${currentUser.id}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${authToken}`,
...
function displayNotifications(notifications) {
  ...
  notificationsContainer.innerHTML = notifications
    .map(
      (notification) => `
        <div class="notification ${notification.nombre_evento || "info"}">
            <p><strong>${"Notificación"}</strong></p>
            <p><strong>Evento:</strong> ${notification.nombre_evento}</p>
            <p><strong>Enviado a:</strong> ${notification.nombre_usuario}</p>
            <small><strong>Fecha:</strong>${formatDate(notification.created_at)}</small>
        </div>
    `
    )
    .join("");
...
function displayMisCompras(purchases) {
  ...
  misComprasContainer.innerHTML = purchases
    .map(
      (purchase) => `
    <div class="purchase" data-purchase-id="${purchase.id}">
        ...
        <p><strong>Estado:</strong> <span class="status ${
          purchase.pagado ? "paid" : "pending"
        }">${purchase.pagado ? "Pagado" : "Pendiente"}</span></p>
        ${
          !purchase.pagado
            ? `<button class="confirm-payment-btn" data-purchase-id="${purchase.id}">Confirmar Pago</button>`
            : ""
        }
    </div>
`
  ...
  document.querySelectorAll(".confirm-payment-btn").forEach((button) => {
    button.addEventListener("click", async (e) => {
      ...
      await confirmPayment(purchaseId);
    });
  });
}
...
async function confirmPayment(purchaseId) {
  try {
    ...
    const response = await fetch(
      `${API_ENDPOINTS.pagos}/confirmar-pago`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    sendNotification({
      ...
      pagado: true,
      total: result.total,
      message: `Pago confirmado para la compra de ${result.cantidad} entrada(s) para ${result.nombre_evento}`,
    });
    await loadMisCompras();
    await loadNotifications();
  } catch (error) {
    console.error("Error:", error);
    alert(error.message || "Ocurrió un error al confirmar el pago");
  }
}

async function sendNotification(notificationData) {
  try {
    await fetch(`${API_ENDPOINTS.notificaciones}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${authToken}`,
